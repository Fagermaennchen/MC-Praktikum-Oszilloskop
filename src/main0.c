...
/********************************************************************************/
void draw7SEG(int mapcode,int size,int y, int x, int color){
    int i;  // Variable for for loops
    int scaledLen = len7Seg*size;
    int scaledWidth = width7Seg*size;
    int scaledArea = len7Seg*width7Seg*(size+1)*(size+1);
    // Draw Segment 0
    if(mapcode & 0x01){
        window_set(x,y,x+scaledLen,y+scaledWidth); // set rectangle position to be Segment 0
        write_command(0x2C); //write pixel command
        for(i=0;i<scaledArea;i++) // set pixels
        {
    ...
    // Draw Segment 1
    if(mapcode & 0x02){
        window_set(x+scaledLen,
                   y+scaledWidth,
                   x+scaledLen+scaledWidth,
                   y+scaledWidth+scaledLen); // set rectangle position to be Segment
        write_command(0x2C); //write pixel command
        for(i=0;i<scaledArea;i++) // set pixels
        {
            write_data((color>>16)&0xff); // red
    ...
    if(mapcode & 0x04){
        window_set(x+scaledLen,
                   y+scaledWidth*2+scaledLen,
                   x+scaledLen+scaledWidth,
                   y+scaledWidth*2+scaledLen*2); // set rectangle position to be Segment
        write_command(0x2C); //write pixel command
        for(i=0;i<scaledArea;i++) // set pixels
        {
            write_data((color>>16)&0xff); // red
...
{  int i,j,x,y;
   ...
   write_command(0x2C); //write pixel command
    ...
    while(1)
    {
        printf("7 Seg\n"); // for debug only
        // Draw Segment 0
        draw7SEG(0x03,1,100,100,WHITE);
        draw7SEG(0x03,2,200,200,WHITE);
        draw7SEG(0x07,3,300,300,WHITE);
    }
}
